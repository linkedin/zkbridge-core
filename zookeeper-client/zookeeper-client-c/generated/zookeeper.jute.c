...
int serialize_PathWithStat(struct oarchive *out, const char *tag, struct PathWithStat *v){
    int rc;
    rc = out->start_record(out, tag);
...
int deserialize_PathWithStat(struct iarchive *in, const char *tag, struct PathWithStat*v){
    int rc;
    rc = in->start_record(in, tag);
...
void deallocate_PathWithStat(struct PathWithStat*v){
...
int serialize_GetChildrenPaginatedRequest(struct oarchive *out, const char *tag, struct GetChildrenPaginatedRequest *v){
    int rc;
    rc = out->start_record(out, tag);
    rc = rc ? rc : out->serialize_String(out, "path", &v->path);
    rc = rc ? rc : out->serialize_Int(out, "maxReturned", &v->maxReturned);
    rc = rc ? rc : out->serialize_Long(out, "minCzxid", &v->minCzxid);
    rc = rc ? rc : out->serialize_Int(out, "czxidOffset", &v->czxidOffset);
    rc = rc ? rc : out->serialize_Bool(out, "watch", &v->watch);
    rc = rc ? rc : out->end_record(out, tag);
    return rc;
}
int deserialize_GetChildrenPaginatedRequest(struct iarchive *in, const char *tag, struct GetChildrenPaginatedRequest*v){
    int rc;
    rc = in->start_record(in, tag);
    rc = rc ? rc : in->deserialize_String(in, "path", &v->path);
    rc = rc ? rc : in->deserialize_Int(in, "maxReturned", &v->maxReturned);
    rc = rc ? rc : in->deserialize_Long(in, "minCzxid", &v->minCzxid);
    rc = rc ? rc : in->deserialize_Int(in, "czxidOffset", &v->czxidOffset);
    rc = rc ? rc : in->deserialize_Bool(in, "watch", &v->watch);
    rc = rc ? rc : in->end_record(in, tag);
    return rc;
}
void deallocate_GetChildrenPaginatedRequest(struct GetChildrenPaginatedRequest*v){
    deallocate_String(&v->path);
}
...
int serialize_GetChildrenPaginatedResponse(struct oarchive *out, const char *tag, struct GetChildrenPaginatedResponse *v){
    int rc;
    rc = out->start_record(out, tag);
    rc = rc ? rc : serialize_String_vector(out, "children", &v->children);
    rc = rc ? rc : serialize_Stat(out, "stat", &v->stat);
    rc = rc ? rc : out->serialize_Long(out, "nextPageCzxid", &v->nextPageCzxid);
    rc = rc ? rc : out->serialize_Int(out, "nextPageCzxidOffset", &v->nextPageCzxidOffset);
    rc = rc ? rc : out->end_record(out, tag);
    return rc;
}
int deserialize_GetChildrenPaginatedResponse(struct iarchive *in, const char *tag, struct GetChildrenPaginatedResponse*v){
    int rc;
    rc = in->start_record(in, tag);
    rc = rc ? rc : deserialize_String_vector(in, "children", &v->children);
    rc = rc ? rc : deserialize_Stat(in, "stat", &v->stat);
    rc = rc ? rc : in->deserialize_Long(in, "nextPageCzxid", &v->nextPageCzxid);
    rc = rc ? rc : in->deserialize_Int(in, "nextPageCzxidOffset", &v->nextPageCzxidOffset);
    rc = rc ? rc : in->end_record(in, tag);
    return rc;
}
void deallocate_GetChildrenPaginatedResponse(struct GetChildrenPaginatedResponse*v){
    deallocate_String_vector(&v->children);
    deallocate_Stat(&v->stat);
}
