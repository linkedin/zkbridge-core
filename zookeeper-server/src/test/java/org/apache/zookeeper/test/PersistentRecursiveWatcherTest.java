...
public class PersistentRecursiveWatcherTest extends ClientBase {
    ...
    public void testNoChildEvents() throws Exception {
        try (ZooKeeper zk = createClient()) {
            ...
            assertEvent(childEvents, EventType.NodeChildrenChanged, "/a", createABStat.getPzxid());

            assertEvent(events, EventType.NodeCreated, "/a/b", createABStat);
            assertEvent(events, EventType.NodeCreated, "/a/b/c", createABCStat);
    ...
    public void testSamePathWithDifferentWatchModes() throws Exception {
        try (ZooKeeper zk = createClient()) {
            ...
            assertEvent(dataEvents, EventType.NodeCreated, "/a", stat);
            assertEvent(persistentEvents, EventType.NodeCreated, "/a", stat);
            assertEvent(recursiveEvents, EventType.NodeCreated, "/a", stat);

            zk.getData("/a", dataEvents::add, null);
            stat = zk.setData("/a", new byte[0], -1);
            assertEvent(dataEvents, EventType.NodeDataChanged, "/a", stat);
            assertEvent(persistentEvents, EventType.NodeDataChanged, "/a", stat);
            assertEvent(recursiveEvents, EventType.NodeDataChanged, "/a", stat);

            zk.getChildren("/a", childEvents::add);
            zk.create("/a/b", new byte[0], ZooDefs.Ids.OPEN_ACL_UNSAFE, CreateMode.PERSISTENT, stat);
            assertEvent(childEvents, EventType.NodeChildrenChanged, "/a", stat);
            assertEvent(persistentEvents, EventType.NodeChildrenChanged, "/a", stat);
            assertEvent(recursiveEvents, EventType.NodeCreated, "/a/b", stat);

            zk.getChildren("/a", childEvents::add);
            zk.delete("/a/b", -1);
            stat = zk.exists("/a", false);
            assertEvent(childEvents, EventType.NodeChildrenChanged, "/a", stat.getPzxid());
            assertEvent(persistentEvents, EventType.NodeChildrenChanged, "/a", stat.getPzxid());
            assertEvent(recursiveEvents, EventType.NodeDeleted, "/a/b", stat.getPzxid());
            ...
            assertEvent(childEvents, EventType.NodeDeleted, "/a", stat.getPzxid());
            assertEvent(dataEvents, EventType.NodeDeleted, "/a", stat.getPzxid());
            assertEvent(dataEvents, EventType.NodeDeleted, "/a", stat.getPzxid());
            assertEvent(persistentEvents, EventType.NodeDeleted, "/a", stat.getPzxid());
            assertEvent(recursiveEvents, EventType.NodeDeleted, "/a", stat.getPzxid());
        }
    }
